...
class TestConversationMessageValidation:
    ...
    def test_from_dict_missing_role(self):
        ...
        data = {"content": "Hello world", "timestamp": "2024-01-01T12:00:00+00:00"}

        with pytest.raises(
            ConversationValidationError, match="Message role is required"
        ):
    ...
    def test_from_dict_invalid_role(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Invalid message role: system"
        ):
            ConversationMessage.from_dict(data)

    def test_from_dict_missing_content(self):
        """Test error handling for missing content."""
        data = {"role": "user", "timestamp": "2024-01-01T12:00:00+00:00"}

        with pytest.raises(
            ConversationValidationError, match="Message content is required"
        ):
            ConversationMessage.from_dict(data)

    def test_from_dict_empty_content_allowed(self):
        """Test that empty string content is allowed."""
        data = {"role": "user", "content": "", "timestamp": "2024-01-01T12:00:00+00:00"}
    ...
    def test_from_dict_content_too_long(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Message content too long"
        ):
            ConversationMessage.from_dict(data)
    ...
    def test_from_dict_missing_timestamp(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Message timestamp is required"
        ):
            ConversationMessage.from_dict(data)
    ...
    def test_from_dict_invalid_timestamp(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Invalid timestamp format"
        ):
            ConversationMessage.from_dict(data)
...
class TestConversationValidation:
    ...
    def test_from_dict_missing_id(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Conversation ID is required"
        ):
    ...
    def test_from_dict_invalid_messages(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Messages must be a list"
        ):
            Conversation.from_dict(data)
    ...
    def test_from_dict_too_many_messages(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Too many messages: 1001 > 1000"
        ):
            Conversation.from_dict(data)
    ...
    def test_add_message_non_string_content(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Message content must be string"
        ):
    ...
    def test_add_message_empty_content(self):
        ...
        with pytest.raises(
            ConversationValidationError, match="Message content cannot be empty"
        ):
