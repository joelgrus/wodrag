...
def mock_agent():
    ...
    mock.forward_verbose.return_value = ("This is a test response.", ["ACTION: search", "OBSERVATION: Found results"])
    return mock


@patch('wodrag.api.routers.agent.get_conversation_service')
def test_agent_query_new_conversation(mock_get_service, client, mock_agent):
    ...
    with patch.object(app.dependencies, 'get', return_value=mock_agent):
...
@patch('wodrag.api.routers.agent.get_conversation_service')
def test_agent_query_existing_conversation(mock_get_service, client, mock_agent):
    ...
    existing_conversation = service.get_or_create_conversation("test-conv-id")
...
@patch('wodrag.api.routers.agent.get_conversation_service')
def test_agent_query_verbose_with_conversation(mock_get_service, client, mock_agent):
    ...
    def mock_forward_verbose(question, conversation_context=None):
        assert conversation_context is not None
        return "Detailed response with reasoning.", ["THOUGHT: Analyzing question", "ACTION: search(Fran)"]
...
@patch('wodrag.api.routers.agent.get_conversation_service')
def test_conversation_persistence(mock_get_service, client, mock_agent):
...
def test_agent_query_error_handling(client):
    ...
    with patch.object(app.dependencies, 'get', return_value=mock_failing_agent):
        response = client.post("/api/v1/agent/query", json={
            "question": "What is Fran?",
            "verbose": False
        })
...
def test_agent_query_validation_errors(client):
