...
from wodrag.database.models import SearchResult, Workout, WorkoutFilter
...
class TestWorkout:
    ...
    def test_workout_to_dict_with_embedding(self) -> None:
        workout = Workout(workout="Test", workout_embedding=[0.1, 0.2, 0.3])
    ...
    def test_workout_from_dict_with_null_date(self) -> None:
        data = {"id": 1, "date": None, "workout": "Rest Day"}
...
class TestWorkoutFilter:
    ...
    def test_filter_to_dict_partial_fields(self) -> None:
        filter_obj = WorkoutFilter(movements=["pull up"], workout_type="metcon")

        result = filter_obj.to_dict()

        assert result == {"movements": ["pull up"], "workout_type": "metcon"}
...
class TestSearchResult:
    def test_search_result_initialization(self) -> None:
        ...
        result = SearchResult(
            workout=workout, similarity_score=0.85, metadata_match=True
        )
    ...
    def test_relevance_score_with_similarity(self) -> None:
        ...
        result1 = SearchResult(
            workout=workout, similarity_score=0.8, metadata_match=True
        )
        assert result1.relevance_score == 0.96  # 0.8 * 1.2

        # High similarity without metadata match
        result2 = SearchResult(
            workout=workout, similarity_score=0.8, metadata_match=False
        )
        assert result2.relevance_score == 0.8

        # Score capped at 1.0
        result3 = SearchResult(
            workout=workout, similarity_score=0.9, metadata_match=True
        )
    ...
    def test_relevance_score_without_similarity(self) -> None:
        ...
        result2 = SearchResult(
            workout=workout, similarity_score=None, metadata_match=False
        )
